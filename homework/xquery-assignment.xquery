...
let $years := data($archive/eadheader/filedesc/titlestmt/titleproper/date)
...
<numberYears>{for $years in data($archive/eadheader/filedesc/titlestmt/titleproper/date)
let $startdate := substring-before($years, '-')
let $enddate := substring-after($years, '-')
return 
numeric-subtract($startdate,$enddate)
(: nooooooope :) 
}</numberYears>
